...
export function RFMQPage() {
  ...
  const handleUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setSegments([]);
      setCustomers([]);
      setAnalysisMeta({ analysisPeriod: "-", customersAnalyzed: 0, transactionsAnalyzed: 0 });
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await apiClient.analyzeRfmq(datasetId, columnMapping, {
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-slate-800/50 rounded-lg">
              ...
              <div className="space-y-3">
                {[
                  { key: "customer_id", label: "customer_id (Customer Identifier)" },
                  { key: "transaction_date", label: "transaction_date (Transaction Date)" },
                  { key: "item_quantity", label: "item_quantity (Item Quantity)" },
                  { key: "unit_price", label: "unit_price (Unit Price)" },
                ].map((field) => (
                  <div
                    key={field.key}
      ...
      {showResults && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <p className="text-lg font-bold text-cyan-300">{analysisMeta.customersAnalyzed.toLocaleString()}</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            <p className="text-xs text-slate-400 mb-1">Transactions Analyzed</p>
            <p className="text-lg font-bold text-slate-100">{analysisMeta.transactionsAnalyzed.toLocaleString()}</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Current: {comparisonMeta.currentActiveCustomers.toLocaleString()} | Previous: {comparisonMeta.previousActiveCustomers.toLocaleString()}
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className={currentPage === 1 ? "bg-slate-700 text-slate-500 cursor-not-allowed" : "bg-cyan-500 hover:bg-cyan-600 text-slate-950 font-semibold"}
              >
                Previous
              </Button>
              {currentPage < totalPages && (
              ...
              <Button
                ...
                className={currentPage === totalPages ? "bg-slate-700 text-slate-500 cursor-not-allowed" : "bg-cyan-500 hover:bg-cyan-600 text-slate-950 font-semibold"}
              >
                Next
              </Button>
            </div>
