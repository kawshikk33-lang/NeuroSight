...
export function AdminPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiClient.get<{ id: string; name?: string; version?: string }>("/models/active")
        .then((model) => {
          if (model?.name) {
        ...
        .catch(() => null),
      apiClient.get<{ last_training_date?: string; last_training_duration?: number }>("/models/training-info")
        .then((info) => {
  ...
  const handleTrainModel = async () => {
    ...
    if (trainingType === "forecasting") {
      ...
      if (!selectedCustomerIdColumn || !selectedDateColumn || !selectedPriceColumn || !selectedQuantityColumn) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            {trainingType === "forecasting" && (
              <>
                <div>
                  <Label className="text-slate-300">Forecast Target (e.g., Sales, Revenue)</Label>
                ...
                <div>
                  ...
                  <div className="mt-2 max-h-32 overflow-y-auto space-y-1 p-2 rounded border border-slate-700 bg-slate-800">
                    {uploadedColumns
                      ...
                      .map((col) => (
                        <label key={col} className="flex items-center gap-2 text-sm text-slate-300">
                          <input
                            type="checkbox"
                ...
                <p className="text-xs text-slate-400">Predict future sales (monthly / quarterly)</p>
              </>
            )}
            ...
            {trainingType === "rfmq" && (
              <>
                <div>
                  ...
                  <Select value={selectedCustomerIdColumn} onValueChange={setSelectedCustomerIdColumn}>
                    <SelectTrigger className="mt-1.5 bg-slate-800 border-slate-700 text-slate-100">
                ...
                <div>
                  ...
                  <Select value={selectedQuantityColumn} onValueChange={setSelectedQuantityColumn}>
                    <SelectTrigger className="mt-1.5 bg-slate-800 border-slate-700 text-slate-100">
                ...
                <p className="text-xs text-slate-400">Segment customers using RFMQ clustering</p>
              </>
            )}

            {uploadedColumns.length === 0 && (
              <p className="text-xs text-slate-400 mt-1">Upload a file to configure training columns</p>
            )}
            ...
            >
              {isTraining ? (
                ...
                <>
                  ...
                  {trainingType === "forecasting" ? "Train Forecast Model" : "Run RFMQ Segmentation"}
                </>
              )}
            </Button>
          </div>
